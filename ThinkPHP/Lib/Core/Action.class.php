            ...
            switch(strtolower($method)) {
                ...
                case '_param'   :  
                    ...
                    if(C('VAR_URL_PARAMS') && isset($_GET[C('VAR_URL_PARAMS')])){
                        $input = array_merge($_GET,$_GET[C('VAR_URL_PARAMS')],$input);//原代码_params不能正确解析post请求里的url参数，$input  =   array_merge($input,$_GET[C('VAR_URL_PARAMS')]);
                    } else {
                        //根据官方论坛的改法改完之后，就可以了，但是一段时间之后又出问题了，发现时isset($_GET[C('VAR_URL_PARAMS')])返回false，所以加上这么一句
